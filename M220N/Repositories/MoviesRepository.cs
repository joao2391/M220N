...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            try
            {
                return await _moviesCollection.Aggregate()
                        ...
                        .FirstOrDefaultAsync();
                       //await _moviesCollection.Aggregate()
                       //.Match(Builders<Movie>.Filter.Eq(x => x.Id, movieId))
                       //.Lookup("comments", movieId, movieId, "movie_comments");
                       //.Lookup("comments", movieId, movieId, "movie_comments")
                       // Ticket: Get Comments
            ...
            )
        {
            ...
            var project = Builders<Movie>.Projection.Include(m => m.Title).Include(m => m.Id);            
            var filter = Builders<Movie>.Filter.In("countries", countries);

            var retorno = await _moviesCollection
            .Find(filter)
            ...
            CancellationToken cancellationToken = default)
        {
           ...
           */
            ...
            var pipeline = new[]
            {
                ...
                //funcionou, menos o rating que está vindo com num 03 e deve ser 02
